  ...
  try {
    ...
    if (deanImageFile) {
      const absolutePath = path.join(
        uploadRoot,
        UPLOAD_SUBDIR,
        deanImageFile.filename
      );
      fs.unlink(absolutePath).catch((cleanError) => {
        console.error(
          `Failed to cleanup file on error: ${deanImageFile.filename}`,
          cleanError.message
        );
      });
    }
...
export const update = async (req, res) => {
  ...
  try {
    ...
    if (deanEducationLevel !== undefined)
      updateData.deanEducationLevel = deanEducationLevel;
    if (deanMessage !== undefined) updateData.deanMessage = deanMessage;

    const existingFaculty = await FacultyService.findFacultyByCode(
      existingFacultyCode
    );
