...
export const list = async (req, res) => {
    try {
        ...
        if (!facultyCode || !departmentCode) {
            return errorResponse(res, "Faculty and Department codes are required", 400);
        }
        ...
        return errorResponse(res, "Internal Server Error while fetching courses", 500);
    }
};
...
export const create = async (req, res) => {
    try {
        ...
        if (!code || !title || credits === undefined) {
            return errorResponse(res, "Missing required fields: code, title, or credits", 400);
        }
        ...
        if (err.code === 'P2002') {
            return errorResponse(res, `A course with code '${req.body.code}' already exists`, 409);
        }
        if (err.message.includes("not found")) {
            return errorResponse(res, err.message, 404);
        }
...
export const update = async (req, res) => {
    try {
        ...
        const updatedCourse = await CourseService.updateCourseByCode(code, updateData);
        return success(res, updatedCourse, "Course updated successfully");
    } catch (err) {
        if (err.code === 'P2025') return errorResponse(res, "Course not found", 404);
        if (err.code === 'P2002') return errorResponse(res, "New course code is already taken", 409);
        return errorResponse(res, err.message, 500);
    }
};
...
export const remove = async (req, res) => {
    try {
        ...
        if (err.code === 'P2003') {
            return errorResponse(res, "Cannot delete: Students are already registered for this course", 409);
        }
        return errorResponse(res, err.message, 500);
    }
};