...
export const list = async (req, res) => {
  try {
    ...
    const { items, total } = await ProgramService.findPrograms(
      skip,
      limit,
      filters
    );
...
export const findProgramByDepartmentCode = async (req, res) => {
  try {
    ...
    const item = await ProgramService.findProgramByDepartmentCode(
      departmentCode
    );
  ...
  try {
    ...
    if (err.message.includes("Invalid value for enum")) {
      return errorResponse(
        res,
        "Invalid Program Type or Delivery Mode provided",
        400
      );
    }
...
export const remove = async (req, res) => {
  try {
    ...
    if (err.code === "P2003")
      return errorResponse(
        res,
        "Cannot delete program; it has linked curriculum or students.",
        409
      );
    return errorResponse(res, err.message);
  }
