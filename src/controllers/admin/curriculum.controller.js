...
export const addToCurriculum = async (req, res) => {
    try {
        ...
        const data = await CurriculumService.addCourseToProgram(programId, courseId, yearLevel, semester);
        return created(res, data, "Course added to curriculum");
    } catch (err) {
        if (err.code === "P2002") return errorResponse(res, "Course already exists in this program", 409);
...
export const getProgramCurriculum = async (req, res) => {
    try {
        ...
        const curriculum = await CurriculumService.getCurriculumByProgram(programId);
...
export const updateCurriculum = async (req, res) => {
    try {
        ...
        const data = await CurriculumService.updateCourseInCurriculum(programId, courseId, { yearLevel, semester });
