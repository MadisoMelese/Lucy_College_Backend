...
export const createCourse = async (payload) => {
    const { code, title, credits, description, tuitionFee, departmentCode } = payload;
    const deptCode = departmentCode.toUpperCase();
    ...
    return await prisma.course.create({
        data: {
            ...
            description,
            tuitionFee: parseFloat(tuitionFee) || 0.00,
...
export const updateCourseByCode = async (existingCode, updateData) => {
    ...
    if (data.departmentCode) {
        ...
        const dept = await prisma.department.findUnique({ where: { departmentCode: data.departmentCode } });
